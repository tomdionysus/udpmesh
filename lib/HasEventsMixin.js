 ...
 * @example
<caption>Emit Events</caption>
this.emit('running', this, true)
 */
class HasEventsMixin extends Mixin {
	 ...
	 * @throws {Exception} 'off: no such event <eventname>' if event not defined
	 */
	on(event, fn) {
	 ...
	 */
	off(event, fn) {
		if(!this._events[event]) throw 'off: no such event '+event
	...
	/**
	 * emit the given event with the specified arguments. Handlers are queued for execution using [setImmediate()]{@link https://developer.mozilla.org/en-US/docs/Web/API/Window/setImmediate}.
	 * @param {String} event The name of the event
	 * @param {...*} args The event arguments
	 * @throws {Exception} 'emit: no such event <eventname>' if event not defined
	 */
	emit(event, ...args) {
		if(!this._events[event]) throw 'emit: no such event '+event
